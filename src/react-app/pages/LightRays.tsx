...
import LightRays, { RaysOrigin } from '../components/effects/LightRays';

const LightRaysPage = () => {
  const [raysOrigin, setRaysOrigin] = useState<RaysOrigin>('top-center');
  const [raysColor, setRaysColor] = useState('#00ffff');
  const [raysSpeed, setRaysSpeed] = useState(1.5);
  const [lightSpread, setLightSpread] = useState(0.8);
  ...
  const preview = (
    <div className="h-[500px] w-full relative bg-[#060010] rounded-lg overflow-hidden border border-gray-800 flex items-center justify-center">
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <h1 className="text-6xl font-black text-white mix-blend-overlay opacity-80 tracking-tighter">LIGHT RAYS</h1>
      </div>
    </div>
  );
  ...
  const customization = (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-gray-400 mb-1">Rays Origin</label>
          <select
            value={raysOrigin}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-400 mb-1">Rays Speed ({raysSpeed})</label>
          <input
            type="range"
            min="0"
            max="5"
            step="0.1"
            value={raysSpeed}
            onChange={(e) => setRaysSpeed(parseFloat(e.target.value))}
            className="w-full bg-gray-800 rounded-lg appearance-none h-2 cursor-pointer"
          />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-400 mb-1">Light Spread ({lightSpread})</label>
          <input
            type="range"
            min="0.1"
            max="2"
        ...
        <div>
          <label className="block text-sm font-medium text-gray-400 mb-1">Ray Length ({rayLength})</label>
          <input
            type="range"
            min="0.1"
            max="2"
      ...
      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-gray-400 mb-1">Mouse Influence ({mouseInfluence})</label>
          <input
            type="range"
            min="0"
            max="1"
        ...
        <div>
          <label className="block text-sm font-medium text-gray-400 mb-1">Noise Amount ({noiseAmount})</label>
          <input
            type="range"
            min="0"
            max="0.5"
        ...
        <div>
          <label className="block text-sm font-medium text-gray-400 mb-1">Distortion ({distortion})</label>
          <input
            type="range"
            min="0"
            max="0.5"
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center justify-between p-3 bg-gray-800/50 rounded-lg border border-gray-700 flex-1 h-10 mt-6">
            <label className="text-sm font-medium text-gray-300">Pulsating</label>
            <input
              type="checkbox"
              checked={pulsating}
              onChange={(e) => setPulsating(e.target.checked)}
...
<div style={{ width: '100%', height: '600px', position: 'relative' }}>
  ...
  const codeExample = `import { useRef, useEffect, useState } from 'react';
...
export default LightRays;`;
  ...
  const props = [
    ...
    },
    {
      ...
      type: 'number',
      default: '1',
      description: 'Spread of the light rays',
    },
    {
      name: 'rayLength',
      type: 'number',
    ...
    },
    {
      ...
      type: 'boolean',
      default: 'false',
      description: 'Enable pulsating effect',
    },
    {
      name: 'mouseInfluence',
      type: 'number',
      default: '0.1',
      description: 'How much the mouse affects ray direction',
    },
  ];
  ...
  return (
    <ComponentDetail
      title="Light Rays"
      description="A volumetric light ray effect using OGL shaders, with mouse interaction and customization."
      preview={preview}
