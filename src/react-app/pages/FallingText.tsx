...
import FallingTextComponent from '../components/text/FallingText';
import Slider from '../components/Slider';

const FallingTextPreview = ({
  trigger = 'hover',
  gravity = 0.56,
  ...
  return (
    <div className="w-full h-[350px] relative">
      ...
      />
      <div className="absolute bottom-2 left-0 right-0 text-center text-gray-400 text-xs pointer-events-none">
        Hover to activate physics • Drag words around
      </div>
    </div>
  );
};

const codeExample = `import { useRef, useState, useEffect } from 'react';
...
const props = [
  ...
  },
  {
    ...
    default: "'auto'",
    description: 'When to trigger the falling effect',
  },
  {
    name: 'gravity',
    type: 'number',
  ...
  },
  {
    ...
    type: 'number',
    default: '0.2',
    description: 'Stiffness of mouse interaction',
  },
  {
    name: 'fontSize',
    type: 'string',
  ...
  },
  {
    ...
    type: 'boolean',
    default: 'false',
    description: 'Show Matter.js wireframes for debugging',
  },
];

export default function FallingText() {
  const [trigger, setTrigger] = useState<'auto' | 'scroll' | 'click' | 'hover'>('hover');
  ...
  const customization = (
    <div className="space-y-6">
      <div>
        <label className="text-sm font-medium text-gray-300 mb-2 block">Trigger</label>
        <select
          value={trigger}
          onChange={(e) => setTrigger(e.target.value as 'auto' | 'scroll' | 'click' | 'hover')}
      ...
      <Slider
        ...
        max={1}
        step={0.1}
        onChange={setMouseConstraintStiffness}
  ...
  return (
    <ComponentDetail
      title="Falling Text"
      description="Interactive text with physics simulation - words fall and can be dragged around using Matter.js"
      preview={<FallingTextPreview trigger={trigger} gravity={gravity} mouseConstraintStiffness={mouseConstraintStiffness} />}
      code={codeExample}
      props={props}
