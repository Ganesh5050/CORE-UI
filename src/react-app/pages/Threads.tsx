  ...
  const preview = (
    <div className="h-[500px] w-full relative bg-[#060010] rounded-lg overflow-hidden border border-gray-800 flex items-center justify-center">
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <h1 className="text-6xl font-black text-white mix-blend-overlay opacity-80 tracking-tighter">THREADS</h1>
      </div>
    </div>
  );
  ...
  const customization = (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-gray-400 mb-1">Amplitude ({amplitude})</label>
          <input
            type="range"
            min="0"
            max="5"
        ...
        <div>
          <label className="block text-sm font-medium text-gray-400 mb-1">Distance ({distance})</label>
          <input
            type="range"
            min="-1"
            max="1"
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center justify-between p-3 bg-gray-800/50 rounded-lg border border-gray-700 flex-1 h-10 mt-6">
            <label className="text-sm font-medium text-gray-300">Mouse Interaction</label>
            <input
              type="checkbox"
...
<div style={{ width: '100%', height: '600px', position: 'relative' }}>
  <Threads
    color={[${hexToRgb(color).map(c => c.toFixed(2)).join(', ')}]}
    amplitude={${amplitude}}
    distance={${distance}}
    enableMouseInteraction={${enableMouseInteraction}}
  />
...
export default Threads;`;
  ...
  const props = [
    {
      ...
      default: '[1, 1, 1]',
      description: 'RGB color array (0-1)',
    },
    {
      name: 'amplitude',
      type: 'number',
  ...
  return (
    <ComponentDetail
      title="Threads"
      description="A flowing thread animation using OGL shaders."
      preview={preview}
