...
export const LaserFlow: React.FC<LaserFlowProps> = ({
  ...
  useEffect(() => {
    console.log('LaserFlow useEffect running');
    const mount = mountRef.current;
    console.log('Mount ref:', mount);
    if (!mount) {
      console.log('Mount ref is null, returning early');
      return;
    }
    console.log('Mount ref exists, creating WebGL renderer...');
    ...
    console.log('Appending canvas to mount:', mount);
    ...
    const animate = () => {
      ...
      // Fade removed - effect appears immediately
    ...
    return () => {
      console.log('LaserFlow cleanup running');
      ...
      material.dispose();
      renderer.dispose();
      console.log('Removing canvas from mount:', mount, 'contains canvas:', mount.contains(canvas));
  ...
  return <div ref={mountRef} className={`laser-flow-container ${className || ''}`} style={{ width: '100%', height: '100%', position: 'relative', pointerEvents: 'none', ...style }} />;
};
